...
async function loadTrendingVideos(nextPageToken = "") {
    const apiKey = "AIzaSyAILgToEFtM2kUSVY5e9blKBCAkX-uJV3M";
    let timestamp = new Date().getTime(); // Unique timestamp to avoid caching
    let url = `https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&chart=mostPopular&regionCode=US&maxResults=10&key=${apiKey}&random=${timestamp}`;
